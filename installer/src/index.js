...
async function discoverAvailableAgents() {
  ...
  try {
    ...
    for (const file of allFiles) {
      ...
      // Skip duplicates (we'll show them with unique names later)
      if (!seen.has(basename)) {
        ...
        agents.push({
          name: friendlyName,
          value: basename,
          description: subdirectory ? `From ${subdirectory}/` : 'Command',
          subdirectory
        });
      }
    }
  ...
  switch (tool.value) {
    case 'claude-code': {
      ...
      const filesToInstall = agents && agents.length > 0 
        ? fileMap.filter(f => agents.includes(f.originalName) || agents.includes(f.destFilename.replace('.md', '')))
        : fileMap;
    ...
    case 'cursor': {
      ...
      const filesToInstall = agents && agents.length > 0 
        ? fileMap.filter(f => agents.includes(f.originalName) || agents.includes(f.destFilename.replace('.mdc', '')))
        : fileMap;
