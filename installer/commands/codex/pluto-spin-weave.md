...
**Core rule:** one thread = one feature. Within a feature, bundle **implementation + fix-ups + tests + feature-local types + imports**. Across features, always split.

**What counts as "one feature":** a single user-facing capability, bug fix, or behavior change. If you can't describe the thread in one sentence without "and," it's probably two features.

**Bundle into one thread (within a single feature):**
- Implementation fiber(s)
- Immediate fix-ups (typos, missing imports, forgotten exports)
- Tests for that feature
- Feature-local types/interfaces
- Import wiring

**Always split into separate threads:**
- Different features (even in the same file)
- Refactors that enabled a feature (ship first, on its own)
- Broadly-used type/interface changes
- Config, dependency, or build changes (always alone)
- Unrelated bug fixes from the same session

**Anti-patterns:** Grouping by "user task" — one request often spans multiple features. Merging fibers just because they touched the same file.
